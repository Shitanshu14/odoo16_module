...
class HrApplicant(models.Model):
    ...
    def action_bulk_validate_email(self):
        ...
        for applicant in self:
            try:
                applicant.is_email_valid = False
                applicant.email_validation_message = False

                if not applicant.email_from:
                    applicant.email_validation_message = "No email provided"
                    continue

                email = applicant.email_from.strip()
                ...
                if config.get('check_smtp', False) and mx_record:
                    ...
                    applicant.is_email_valid = result
                    applicant.email_validation_message = "Valid Email" if result else "User Does Not Exist"
                else:
                    applicant.email_validation_message = "Ready for SMTP Check" if mx_record else "DNS Skipped"

            except Exception as e:
                _logger.warning(f"Validation error for {applicant.email_from}: {e}")
                applicant.email_validation_message = str(e)

            applicant._compute_show_validate_email_button()
