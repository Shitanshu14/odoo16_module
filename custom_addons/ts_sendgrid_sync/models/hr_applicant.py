...
class HrApplicant(models.Model):
    ...
    display_warning_applicant = fields.Boolean(string='Display Warning', compute='_compute_display_warning', store=False)

    @api.depends()
    def _compute_display_warning(self):
        config_settings = self.env['res.config.settings'].default_get(['display_warning_applicant'])
        display_warning_applicant = config_settings.get('display_warning_applicant', False)
        for partner in self:
            partner.display_warning_applicant = display_warning_applicant

    @api.model
    def create(self, vals_list):
        res = super(HrApplicant, self).create(vals_list)
        auto_sync_applicant = self.env['ir.config_parameter'].sudo().get_param('ts_sendgrid_sync.auto_sync_applicant', default=False)
        if auto_sync_applicant == 'True':
    ...
    def unlink(self):
        applicants = []
        delete_sync_applicant = self.env['ir.config_parameter'].sudo().get_param('ts_sendgrid_sync.delete_sync_applicant', default=False)
        if delete_sync_applicant == 'True':
            for rec in self:
                applicants.append(rec.id)
            self.delete_contact_in_sendgrid(applicants)
        return super(HrApplicant, self).unlink()


    def delete_contact_in_sendgrid(self, applicants):
        api_key = self.get_key()
        sg = sendgrid.SendGridAPIClient(api_key.get('SENDGRID_API_KEY'))

        records = self.env['hr.applicant'].browse(applicants)
    ...
    def button_to_link(self):
        ...
        return {
            ...
            'res_model': 'applicant.fields.link',
            'view_mode': 'tree',
            'view_id': self.env.ref('ts_sendgrid_sync.applicant_fields_link_view_Tree').id,
            'target': 'current',
        }
    ...
    def add_contact_from_odoo(self):
        return {
            ...
            'res_model': 'applicant.send.wizard',
            'view_mode': 'form',
            'target': 'new',
        }
        # for record in self:
